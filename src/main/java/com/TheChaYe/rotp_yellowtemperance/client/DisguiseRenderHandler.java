...
//            LOGGER.error("初始化反射字段失败", e);
    ...
    public static void onPreRenderLiving(RenderLivingEvent.Pre<? extends LivingEntity, ? extends EntityModel<?>> event) {
        ...
        IStandPower.getStandPowerOptional(player).ifPresent(standPower -> {
            ...
            handlePlayerDisguise(player, disguiseStand, event);
        });
    }
    ...
    private static void handlePlayerDisguise(PlayerEntity player, AbstractDisguiseStandEntity stand, RenderLivingEvent.Pre<?, ?> event) {
        UUID playerId = player.getUUID();
...
//            LOGGER.error("调整碰撞箱失败", e);
            ...
            if (disguise != null) {
                ...
                IStandPower.getStandPowerOptional(player).ifPresent(standPower -> {
                    if (standPower.getStandManifestation() instanceof AbstractDisguiseStandEntity) {
                        AbstractDisguiseStandEntity disguiseStand = (AbstractDisguiseStandEntity) standPower.getStandManifestation();
                        // 确保物理属性更新
                        adjustPlayerPhysics(player, disguise, disguiseStand);
                    }
                });
            }
        } else {
    ...
    private static void adjustPlayerPhysics(PlayerEntity player, LivingEntity disguise, AbstractDisguiseStandEntity stand) {
...
//                LOGGER.error("注意：反射调用在生产环境中可能因字段名混淆而失效");
    ...
    /**
     * 玩家断开连接事件 / Player disconnect event
     * 清理玩家的伪装数据，防止内存泄漏
     *
     * @param event 玩家离开世界事件 / Player leave world event
     */
    @SubscribeEvent
    @OnlyIn(Dist.CLIENT)
    public static void onPlayerLeaveWorld(PlayerEvent.PlayerLoggedOutEvent event) {
        ...
        // 清理该玩家的伪装数据
        cleanupPlayerDisguise(playerId);
    }

    /**
     * 玩家改变维度事件 / Player change dimension event
     * 清理跨维度的伪装数据
     *
     * @param event 玩家改变维度事件 / Player change dimension event
     */
    @SubscribeEvent
    @OnlyIn(Dist.CLIENT)
    ...
    /**
     * 清理单个玩家的伪装数据 / Cleanup single player disguise data
     *
     * @param playerId 玩家 ID / Player ID
     */
    private static void cleanupPlayerDisguise(UUID playerId) {
        ...
        ORIGINAL_EYE_HEIGHTS.remove(playerId);
        ORIGINAL_DIMENSIONS.remove(playerId);
        ORIGINAL_SHADOW_SIZES.remove(playerId);
        LAST_EYE_HEIGHT_SCALES.remove(playerId);
        LAST_HITBOX_WIDTH_SCALES.remove(playerId);
        LAST_HITBOX_HEIGHT_SCALES.remove(playerId);
        PLAYER_DOLLS.remove(playerId);

        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("已清理玩家 {} 的伪装数据", playerId);
        }
    }

    /**
     * 清理所有伪装数据和实体 / Cleanup all disguise data and entities
     */
    private static void cleanupAllDisguises() {
        // 清理所有伪装实体
        ...
        ORIGINAL_EYE_HEIGHTS.clear();
        ORIGINAL_DIMENSIONS.clear();
        ORIGINAL_SHADOW_SIZES.clear();
        PLAYER_DOLLS.clear();
        LAST_EYE_HEIGHT_SCALES.clear();
        LAST_HITBOX_WIDTH_SCALES.clear();
        LAST_HITBOX_HEIGHT_SCALES.clear();

        LOGGER.info("已清理所有伪装数据和实体");
    }
