...
public class ClientSyncNbtPacket {
    ...
    public static void handle(ClientSyncNbtPacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            // 服务器端处理 / Server side handling
            if (ctx.get().getDirection().getReceptionSide().isServer()) {
                net.minecraft.entity.player.ServerPlayerEntity serverPlayer = ctx.get().getSender();
                if (serverPlayer != null && serverPlayer.getUUID().equals(msg.playerId)) {
                    ...
                    if (msg.nbtData.contains(InitTags.YT_DISGUISE_LOADING_TAG)) {
                        ...
                        serverPlayer.getPersistentData().putBoolean(InitTags.YT_DISGUISE_LOADING_TAG, isLoading);
                        ...
                        syncData.putBoolean(InitTags.YT_HAS_DISGUISE_TAG, serverPlayer.getPersistentData().getBoolean(InitTags.YT_HAS_DISGUISE_TAG));

                        com.TheChaYe.rotp_yellowtemperance.network.ModNetwork.sendToAllTracking(
                                new com.TheChaYe.rotp_yellowtemperance.network.packets.server.SyncNbtToClientPacket(serverPlayer.getUUID(), syncData),
                                serverPlayer
                        );

                        RotPYellowTemperanceAddon.LOGGER.debug("Updated disguise loading status for player {}: {}",
                                serverPlayer.getName().getString(), isLoading ? "loading" : "complete");
                    }
                }
                return;
            }
